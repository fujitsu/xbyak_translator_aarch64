...
unsigned int xed_convert_ascii_to_hex(const char *src, xed_uint8_t *dst,
                                      unsigned int max_bytes);

#define XED_HEX_BUFLEN 200
void xed_print_hex_line(char *buf, const xed_uint8_t *array,
                        const unsigned int length, const unsigned int buflen);
...
void xed_map_region(const char *path, void **start, unsigned int *length);
...
void xed_register_disassembly_callback(xed_disassembly_callback_fn_t f);

void disassemble(xed_disas_info_t *di, char *buf, int buflen,
                 xed_decoded_inst_t *xedd,
                 xed_uint64_t runtime_instruction_address, void *caller_data);
...
void init_xedd(xed_decoded_inst_t *xedd, xed_disas_info_t *di);
