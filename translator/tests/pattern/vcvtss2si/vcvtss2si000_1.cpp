...
public:
  ...
  void genJitTestCode() {
    ...
    std::vector<mkldnn_round_mode_t> rmode = {
        ...
        mkldnn_round_zero};
    std::vector<Xbyak::EvexModifierRounding> sae = {T_rn_sae, T_rd_sae,
                                                    T_ru_sae, T_rz_sae};
    int j = 0;

    for (const auto &e : rmode) {
      set_rnd_mode(
          e); /* This setup makes no sence, because the rounding mode set by
                 operands. This is to make sure the MXCSR flags is ignored. */
      for (int i = 0; i < 16; i++) {
