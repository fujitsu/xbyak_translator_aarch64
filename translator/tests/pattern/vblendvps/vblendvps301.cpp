...
public:
  ...
  void genJitTestCode() {
    ...
    mov(rax, addr);
    vblendvps(Xmm(0), Xmm(0), ptr[rax], Xmm(11));
    vblendvps(Xmm(2), Xmm(2), ptr[rax], Xmm(12));
    vblendvps(Xmm(3), Xmm(3), ptr[rax], Xmm(13));

    addr = reinterpret_cast<size_t>(&(inputZReg[4].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Xmm(4), Xmm(7), ptr[rax], Xmm(11));
    addr = reinterpret_cast<size_t>(&(inputZReg[5].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Xmm(5), Xmm(7), ptr[rax], Xmm(12));
    addr = reinterpret_cast<size_t>(&(inputZReg[6].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Xmm(6), Xmm(7), ptr[rax], Xmm(13));
