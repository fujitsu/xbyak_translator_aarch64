...
public:
  ...
  void genJitTestCode() {
    ...
    vblendvps(Xmm(2), Xmm(0), ptr[rax], Xmm(11));
    vblendvps(Xmm(3), Xmm(0), ptr[rax], Xmm(12));
    vblendvps(Xmm(4), Xmm(0), ptr[rax], Xmm(13));

    vblendvps(Xmm(5), Xmm(5), ptr[rax], Xmm(11));
    vblendvps(Xmm(6), Xmm(6), ptr[rax], Xmm(12));
    vblendvps(Xmm(7), Xmm(7), ptr[rax], Xmm(13));

    addr = reinterpret_cast<size_t>(&(inputZReg[8].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Xmm(8), Xmm(0), ptr[rax], Xmm(11));
    addr = reinterpret_cast<size_t>(&(inputZReg[9].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Xmm(9), Xmm(0), ptr[rax], Xmm(12));
    addr = reinterpret_cast<size_t>(&(inputZReg[10].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Xmm(10), Xmm(0), ptr[rax], Xmm(13));
