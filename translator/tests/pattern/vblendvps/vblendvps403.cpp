...
public:
  ...
  void genJitTestCode() {
    ...
    mov(rax, addr);
    vblendvps(Ymm(2), Ymm(1), ptr[rax], Ymm(11));
    addr = reinterpret_cast<size_t>(&(inputZReg[12].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Ymm(3), Ymm(1), ptr[rax], Ymm(12));
    addr = reinterpret_cast<size_t>(&(inputZReg[13].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Ymm(4), Ymm(1), ptr[rax], Ymm(13));

    addr = reinterpret_cast<size_t>(&(inputZReg[1].ud_dt[0]));
    mov(rax, addr);
    vblendvps(Ymm(5), Ymm(11), ptr[rax], Ymm(11));
    vblendvps(Ymm(6), Ymm(12), ptr[rax], Ymm(12));
    vblendvps(Ymm(7), Ymm(13), ptr[rax], Ymm(13));
