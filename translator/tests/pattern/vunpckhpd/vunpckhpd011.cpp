...
public:
  ...
  void genJitTestCode() {
    ...
    vunpckhpd(Xmm(0)|k1|T_z, Xmm(1), ptr_b[rax]);
    vunpckhpd(Xmm(2)|k2|T_z, Xmm(2), ptr_b[rax]);

    vunpckhpd(Ymm(3)|k3|T_z, Ymm(4), ptr_b[rax]);
    vunpckhpd(Ymm(5)|k4|T_z, Ymm(5), ptr_b[rax]);

    vunpckhpd(Zmm(6)|k5|T_z, Zmm(7), ptr_b[rax]);
    vunpckhpd(Zmm(8)|k6|T_z, Zmm(8), ptr_b[rax]);
