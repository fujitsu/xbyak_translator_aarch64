...
/* Address is aligned */
    ...
    mov(rbx, addr);
    vpmovdb(ptr[rbx], Ymm(0)); // truncate
    vmovdqu8(Zmm(1), ptr[rbx]);
    mov(rbx, addr1);
    vpmovdb(ptr[rbx], Ymm(2)); // no truncate
    vmovdqu8(Zmm(3), ptr[rbx]);
    mov(rbx, addr2);
    vpmovdb(ptr[rbx], Ymm(4)); // random
...
/* Address is unaligned */
    ...
    mov(rbx, addr);
    vpmovdb(ptr[rbx], Ymm(6));
    vmovdqu8(Zmm(7), ptr[rbx]);
    mov(rbx, addr1);
    vpmovdb(ptr[rbx], Ymm(8));
    vmovdqu8(Zmm(9), ptr[rbx]);
    mov(rbx, addr2);
    vpmovdb(ptr[rbx], Ymm(10));
