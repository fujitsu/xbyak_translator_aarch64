...
public:
...
#ifndef __ARM_ARCH
    inputPredReg[1] = (1 << 0) | (1 << 2) | (1 << 4) | (1 << 6) | (1 << 8) | (1 << 10) | (1 << 12) | (1 << 14);
    inputPredReg[2] = (1 << 1) | (1 << 3) | (1 << 5) | (1 << 7) | (1 << 9) | (1 << 11) | (1 << 13) | (1 << 15);
    inputPredReg[3] = ~uint64_t(0);
    inputPredReg[4] = (1 << 1) | (1 << 3) | (1 << 4) | (1 << 6) | (1 << 9) | (1 << 11) | (1 << 12) | (1 << 14);
#else
    inputPredReg[1] = (1 << 0) | (1 << 8) | (1 << 16) | (1 << 24) | (uint64_t(1) << 32) | (uint64_t(1) << 40) | (uint64_t(1) << 48) | (uint64_t(1) << 56);
    inputPredReg[2] = (1 << 4) | (1 << 12) | (1 << 20) | (uint64_t(1) << 28) | (uint64_t(1) << 36) | (uint64_t(1) << 44) | (uint64_t(1) << 52) | (uint64_t(1) << 60);
    inputPredReg[3] = ~uint64_t(0);
    inputPredReg[4] = (1 << 4) | (1 << 12) | (1 << 16) | (1 << 24) | (uint64_t(1) << 36) | (uint64_t(1) << 44) | (uint64_t(1) << 48) | (uint64_t(1) << 56);
#endif
  }
  ...
  void genJitTestCode() {
    ...
    mov(rax, addr1);
    vpxord(Xmm(2) | k1, Xmm(0), ptr[rax]);
    mov(rax, addr2);
    vpxord(Ymm(5) | k1, Ymm(3), ptr[rax]);
    mov(rax, addr3);
    vpxord(Zmm(8) | k1, Zmm(6), ptr[rax]);

    mov(rax, addr4);
    vpxord(Xmm(6) | k2, Xmm(7), ptr[rax]);
    mov(rax, addr5);
    vpxord(Xmm(8) | k3, Xmm(9), ptr[rax]);
    mov(rax, addr6);
    vpxord(Xmm(10) | k4, Xmm(10), ptr[rax]);

    mov(rax, addr7);
    vpxord(Ymm(13) | k2, Ymm(14), ptr[rax]);
    mov(rax, addr8);
    vpxord(Ymm(15) | k3, Ymm(16), ptr[rax]);
    mov(rax, addr9);
    vpxord(Ymm(17) | k4, Ymm(17), ptr[rax]);

    mov(rax, addr10);
    vpxord(Zmm(21) | k2, Zmm(22), ptr[rax]);
    mov(rax, addr11);
    vpxord(Zmm(23) | k3, Zmm(24), ptr[rax]);
    mov(rax, addr12);
    vpxord(Zmm(25) | k4, Zmm(25), ptr[rax]);

    mov(rax, addr13);
    vpxord(Xmm(29) | k1, Xmm(29), ptr[rax]);
    mov(rax, addr14);
    vpxord(Ymm(30) | k1, Ymm(30), ptr[rax]);
    mov(rax, addr15);
    vpxord(Zmm(31) | k1, Zmm(31), ptr[rax]);
