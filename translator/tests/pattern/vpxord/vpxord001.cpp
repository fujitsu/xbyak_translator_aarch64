...
public:
  ...
  void genJitTestCode() {
    ...
    mov(rax, addr1);
    vpxord(Xmm(2), Xmm(0), ptr[rax]);
    mov(rax, addr2);
    vpxord(Ymm(5), Ymm(3), ptr[rax]);
    mov(rax, addr3);
    vpxord(Zmm(8), Zmm(6), ptr[rax]);

    mov(rax, addr4);
    vpxord(Xmm(6), Xmm(7), ptr[rax]);
    mov(rax, addr5);
    vpxord(Xmm(8), Xmm(9), ptr[rax]);
    mov(rax, addr6);
    vpxord(Xmm(10), Xmm(10), ptr[rax]);

    mov(rax, addr7);
    vpxord(Ymm(13), Ymm(14), ptr[rax]);
    mov(rax, addr8);
    vpxord(Ymm(15), Ymm(16), ptr[rax]);
    mov(rax, addr9);
    vpxord(Ymm(17), Ymm(17), ptr[rax]);

    mov(rax, addr10);
    vpxord(Zmm(21), Zmm(22), ptr[rax]);
    mov(rax, addr11);
    vpxord(Zmm(23), Zmm(24), ptr[rax]);
    mov(rax, addr12);
    vpxord(Zmm(25), Zmm(25), ptr[rax]);

    mov(rax, addr13);
    vpxord(Xmm(29), Xmm(29), ptr[rax]);
    mov(rax, addr14);
    vpxord(Ymm(30), Ymm(30), ptr[rax]);
    mov(rax, addr15);
    vpxord(Zmm(31), Zmm(31), ptr[rax]);
