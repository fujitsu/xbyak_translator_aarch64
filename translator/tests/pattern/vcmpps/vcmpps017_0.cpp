...
public:
  ...
  void genJitTestCode() {
    ...
    vcmpps(k1 | k6, Xmm(0), ptr_b[rax], 0);  // EQ_OQ
    vcmpps(k2 | k6, Xmm(2), ptr_b[rcx], 0);  // EQ_OQ
    vcmpps(k3 | k6, Xmm(4), ptr_b[rdx], 0);  // EQ_OQ
    vcmpps(k7 | k6, Xmm(31), ptr_b[rbx], 0); // EQ_OQ

    vcmpps(k6 | k6, Xmm(6), ptr_b[rbp], 0); // EQ_OQ
    vcmpps(k5 | k5, Xmm(8), ptr_b[rsi], 0); // EQ_OQ
