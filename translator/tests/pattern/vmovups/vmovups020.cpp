...
public:
  ...
  void genJitTestCode() {
    ...
    vmovups(ptr[rax]|k1, Zmm(0));
    vmovups(Zmm(1), ptr[rax]);

    vmovups(ptr[rax]|k2, Zmm(0));
    vmovups(Zmm(2), ptr[rax]);

    vmovups(ptr[rax]|k3, Zmm(0));
    vmovups(Zmm(3), ptr[rax]);

    vmovups(ptr[rax]|k4, Zmm(0));
    vmovups(Zmm(4), ptr[rax]);

    vmovups(ptr[rax]|k5, Zmm(0));
    vmovups(Zmm(5), ptr[rax]);

    vmovups(ptr[rax]|k6, Zmm(0));
    vmovups(Zmm(6), ptr[rax]);

    vmovups(ptr[rax]|k7, Zmm(0));
