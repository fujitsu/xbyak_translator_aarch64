...
public:
  ...
  void genJitTestCode() {
    ...
    vunpcklps(Xmm(0) | k1, Xmm(1), ptr_b[rax]);
    vunpcklps(Xmm(2) | k2, Xmm(2), ptr_b[rax]);

    vunpcklps(Ymm(3) | k3, Ymm(4), ptr_b[rax]);
    vunpcklps(Ymm(5) | k4, Ymm(5), ptr_b[rax]);

    vunpcklps(Zmm(6) | k5, Zmm(7), ptr_b[rax]);
    vunpcklps(Zmm(8) | k6, Zmm(8), ptr_b[rax]);
