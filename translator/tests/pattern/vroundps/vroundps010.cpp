...
public:
  ...
  void genJitTestCode() {
    ...
    size_t addr = reinterpret_cast<size_t>(&(inputZReg[0].ud_dt[0]));

    /* Register index is VEX range. */
    mov(rax, addr);
    vroundps(Xmm(8), ptr[rax + 64 * 0], rounding_direction);
    vroundps(Xmm(9), ptr[rax + 64 * 1], rounding_direction);
    vroundps(Xmm(10), ptr[rax + 64 * 2], rounding_direction);
    vroundps(Xmm(11), ptr[rax + 64 * 3], rounding_direction);
    vroundps(Xmm(12), ptr[rax + 64 * 4], rounding_direction);
    vroundps(Xmm(13), ptr[rax + 64 * 5], rounding_direction);
    vroundps(Xmm(14), ptr[rax + 64 * 6], rounding_direction);
    vroundps(Xmm(15), ptr[rax + 64 * 7], rounding_direction);
