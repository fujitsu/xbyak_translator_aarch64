...
public:
  void setInitialRegValue() {
    ...
    inputZReg[31].sd_dt[0] = std::numeric_limits<int64_t>::max();
    inputZReg[31].sd_dt[1] = std::numeric_limits<int64_t>::min();
    inputZReg[31].sd_dt[2] = 0;
    inputZReg[31].ud_dt[3] = std::numeric_limits<uint64_t>::max();
    inputZReg[31].ud_dt[4] = std::numeric_limits<uint64_t>::min();
  }
  ...
  void genJitTestCode() {
    ...
    for (int j = 0; j < 1; j++) {
      ...
      for (int i = 0; i < 5; i++) {
        size_t srcAddr = reinterpret_cast<size_t>(&(inputZReg[31].ud_dt[i]));
        xor_(rax, rax); // clear rax
        mov(rax, srcAddr);
        cmp(qword[rax], std::numeric_limits<uint32_t>::min());
#ifdef XBYAK_TRANSLATE_AARCH64
