...
public:
  ...
  void genJitTestCode() {
    ...
    /* z31 - z29 are used as zTmpIdx - zTmp3Idx */
    vptestmd(k4 | k1, Xmm(0), ptr[rax]);
    vptestmd(k5 | k2, Xmm(2), ptr[rax]);
    vptestmd(k6 | k3, Xmm(4), ptr[rcx]);

    vptestmd(k1 | k1, Xmm(6), ptr[rcx]);
    vptestmd(k2 | k2, Xmm(8), ptr[rax]);
    vptestmd(k3 | k3, Xmm(10), ptr[rax]);
