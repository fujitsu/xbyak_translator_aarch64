...
public:
  ...
  void genJitTestCode() {
    ...
    /* z31 - z29 are used as zTmpIdx - zTmp3Idx */
    vptestmd(k4 | k1, Zmm(0), ptr[rax]);
    vptestmd(k5 | k2, Zmm(2), ptr[rax]);
    vptestmd(k6 | k3, Zmm(4), ptr[rcx]);

    vptestmd(k1 | k1, Zmm(6), ptr[rcx]);
    vptestmd(k2 | k2, Zmm(8), ptr[rax]);
    vptestmd(k3 | k3, Zmm(10), ptr[rax]);
