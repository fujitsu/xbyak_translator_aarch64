...
public:
  ...
  void genJitTestCode() {
    ...
    mov(rax, addr1);
    vpxorq(Xmm(2), Xmm(0), ptr[rax]);
    mov(rax, addr2);
    vpxorq(Ymm(5), Ymm(3), ptr[rax]);
    mov(rax, addr3);
    vpxorq(Zmm(8), Zmm(6), ptr[rax]);

    mov(rax, addr4);
    vpxorq(Xmm(6), Xmm(7), ptr[rax]);
    mov(rax, addr5);
    vpxorq(Xmm(8), Xmm(9), ptr[rax]);
    mov(rax, addr6);
    vpxorq(Xmm(10), Xmm(10), ptr[rax]);

    mov(rax, addr7);
    vpxorq(Ymm(13), Ymm(14), ptr[rax]);
    mov(rax, addr8);
    vpxorq(Ymm(15), Ymm(16), ptr[rax]);
    mov(rax, addr9);
    vpxorq(Ymm(17), Ymm(17), ptr[rax]);

    mov(rax, addr10);
    vpxorq(Zmm(21), Zmm(22), ptr[rax]);
    mov(rax, addr11);
    vpxorq(Zmm(23), Zmm(24), ptr[rax]);
    mov(rax, addr12);
    vpxorq(Zmm(25), Zmm(25), ptr[rax]);

    mov(rax, addr13);
    vpxorq(Xmm(29), Xmm(29), ptr[rax]);
    mov(rax, addr14);
    vpxorq(Ymm(30), Ymm(30), ptr[rax]);
    mov(rax, addr15);
    vpxorq(Zmm(31), Zmm(31), ptr[rax]);
