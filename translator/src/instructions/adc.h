 ...
 *******************************************************************************/
void Xbyak::CodeGenerator::translateADC(xed_decoded_inst_t *p) {
  ...
  if (false || (a64.dstWidth == 32 && a64.dstType == A64_OP_REG &&
    ...
    xa_->adc(xa::WReg(a64.dstIdx), xa::WReg(a64.dstIdx), W_TMP_0);
  }
  /* Col=U103*/
  if (false || (a64.dstWidth == 64 && a64.dstType == A64_OP_REG &&
                a64.srcType == A64_OP_IMM && true)) {
    xa_->mov_imm(X_TMP_0, a64.uimm);
    xa_->adc(xa::XReg(a64.dstIdx), xa::XReg(a64.dstIdx), X_TMP_0);
  }
  ...
  if (false || (a64.dstWidth == 32 && a64.dstType == A64_OP_MEM &&
    ...
    xa_->mov_imm(W_TMP_1, static_cast<uint32_t>(a64.uimm));
    xa_->ldr(W_TMP_0, xa::ptr(X_TMP_ADDR));
    xa_->adc(W_TMP_0, W_TMP_0, W_TMP_1);
  }
  /* Col=AC103*/
  if (false || (a64.dstWidth == 64 && a64.dstType == A64_OP_MEM &&
                a64.srcType == A64_OP_IMM && true)) {
    xa_->mov_imm(X_TMP_1, static_cast<uint32_t>(a64.uimm));
    xa_->ldr(X_TMP_0, xa::ptr(X_TMP_ADDR));
    xa_->adc(X_TMP_0, X_TMP_0, X_TMP_1);
  }

  /* Col=AF103*/
  if (false || (a64.dstWidth == 32 && a64.dstType == A64_OP_REG &&
                a64.srcType == A64_OP_REG && true)) {
    xa_->adc(xa::WReg(a64.dstIdx), xa::WReg(a64.dstIdx), xa::WReg(a64.srcIdx));
  }
  /* Col=AG103*/
  if (false || (a64.dstWidth == 64 && a64.dstType == A64_OP_REG &&
                a64.srcType == A64_OP_REG && true)) {
    xa_->adc(xa::XReg(a64.dstIdx), xa::XReg(a64.dstIdx), xa::XReg(a64.srcIdx));
  }

  /* Col=AK103*/
  if (false || (a64.dstWidth == 32 && a64.dstType == A64_OP_MEM &&
                a64.srcType == A64_OP_REG && true)) {
    xa_->adc(W_TMP_0, W_TMP_0, xa::WReg(a64.srcIdx));
  }
  /* Col=AL103*/
  if (false || (a64.dstWidth == 64 && a64.dstType == A64_OP_MEM &&
                a64.srcType == A64_OP_REG && true)) {
    xa_->adc(X_TMP_0, X_TMP_0, xa::XReg(a64.srcIdx));
  }
