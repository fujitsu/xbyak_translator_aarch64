...
void Xbyak::CodeGenerator::translateCMOVNLE(xed_decoded_inst_t *p) {
  ...
  if (false ||
      ...
      (a64.operands[1].opName == XED_OPERAND_MEM0 &&
       a64.operands[0].opWidth == 32 && true)) {
    xa_->csel(xa::WReg(dstIdx), xa::WReg(srcIdx), xa::WReg(dstIdx), xa::GT);
  }
  /* Col=AL119*/
  if (false ||
      (a64.operands[1].opName == XED_OPERAND_REG1 &&
      ...
      (a64.operands[1].opName == XED_OPERAND_MEM0 &&
       a64.operands[0].opWidth == 64 && true)) {
    xa_->csel(xa::XReg(dstIdx), xa::XReg(srcIdx), xa::XReg(dstIdx), xa::GT);
  }
  ...
  if (false ||
      ...
      (a64.operands[1].opName == XED_OPERAND_MEM0 &&
       a64.operands[0].opWidth == 16 && true)) {
    xa_->csel(xa::XReg(dstIdx), X_TMP_1, xa::XReg(dstIdx), xa::GT);
  }
